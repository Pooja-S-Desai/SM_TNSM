...
def get_min_controllers_and_assignment(
    ...
    **kwargs,             # swallow any future kwargs safely
    ...
    if num_nodes > 25 :
        m.addConstr(quicksum(x1[v] for v in G.nodes()) >= 4, name="min_4_controllers")
    else:
        m.addConstr(quicksum(x1[v] for v in G.nodes()) >= 3, name="min_3_controllers")
    ...
    if num_nodes < 15:
        ...
        min_overloaded_ctrls = 2
    else:
        min_overloaded_ctrls = max(
            1,
            int(0.3 * num_nodes)
        )
    ...
    m.addConstr(
        ...
        name=f"min_{min_overloaded_ctrls}_overloaded_controllers"
    )
    for v in G.nodes():
